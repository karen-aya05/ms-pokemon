...
La función Lambda `lda-CreatePokemon` recibe el nombre de un Pokémon, consulta sus datos en la API pública de Pokémon (https://pokeapi.co/api/v2/pokemon/{name}), y guarda la información relevante en una tabla DynamoDB.
...
- Internos
  ...
  - `LOCAL_AWS_PROFILE_NAME`: Perfil de AWS para uso local
  - `AWS_REGION`: Región AWS (ejemplo: `us-east-1`)
  - `POKEMON_API_BASE_URL`: URL base de la API pública de Pokémon 
...
POKEMON_API_BASE_URL=https://pokeapi.co
